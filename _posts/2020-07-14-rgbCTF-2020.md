...
Oh. Vậy ra cần phải có 1 số random để sinh 1 số random, magic :v

![](../ctf/rgbCTF-2020/img/weird-als-kitchen-magic.gif)
...
Gọi `u` là `intersection 1`, `v` là `intersection 2`, `w` là `distance`, `s` là `string on the road`. Với mỗi con đường mình tạo 2 cạnh ngược chiều nhau với cùng một trọng số và chuỗi s:
